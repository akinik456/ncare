...
class _RequesterScreenState extends State<RequesterScreen> with SingleTickerProviderStateMixin{
  ...
  StreamSubscription<QuerySnapshot<Map<String, dynamic>>>? _approvedLocatorSub;
...
print('myLat $_myLat , myLng $_myLng');
  ...
  void _listenApprovedLocators() {
    ...
    _approvedLocatorSub?.cancel();
    _approvedLocatorSub = FirebaseFirestore.instance
        .collection('locators')
        .where('pairedRequesterId', isEqualTo: id)
        .snapshots()
        .listen((snapshot) {
      for (final doc in snapshot.docs) {
        _syncApprovedLocator(doc);
      }
    });
  }

  Future<void> _syncApprovedLocator(
    QueryDocumentSnapshot<Map<String, dynamic>> locatorDoc,
  ) async {
    final id = requesterId;
    if (id == null || id.isEmpty) return;

    final locatorId = locatorDoc.id;
    final locatorData = locatorDoc.data();
    final locatorName = (locatorData['name'] ?? 'Locator').toString().trim();

    final requesterLocatorRef = FirebaseFirestore.instance
        .collection('requesters')
        .doc(id)
        .collection('locators')
        .doc(locatorId);

    final requesterLocatorDoc = await requesterLocatorRef.get();
    final requesterLocatorData = requesterLocatorDoc.data();
    final isAlreadyActive = requesterLocatorData?['active'] == true;
    final existingName = (requesterLocatorData?['name'] ?? '').toString().trim();

    if (isAlreadyActive && existingName == locatorName) {
      return;
    }

    final prefs = await SharedPreferences.getInstance();
    final batteryThreshold = prefs.getInt('batteryAlertThreshold') ?? 20;

    await requesterLocatorRef.set({
      'name': locatorName,
      'active': true,
      'callEnabled': requesterLocatorData?['callEnabled'] ?? true,
      'batteryAlarmEnabled': requesterLocatorData?['batteryAlarmEnabled'] ?? true,
      'batteryAlertThreshold': requesterLocatorData?['batteryAlertThreshold'] ?? batteryThreshold,
      'gpsOffAlarmEnabled': requesterLocatorData?['gpsOffAlarmEnabled'] ?? false,
      'geofenceAlarmEnabled': requesterLocatorData?['geofenceAlarmEnabled'] ?? false,
      'geofenceRadius': requesterLocatorData?['geofenceRadius'] ?? 250,
      'createdAt': requesterLocatorData?['createdAt'] ?? FieldValue.serverTimestamp(),
      'pairedAt': FieldValue.serverTimestamp(),
    }, SetOptions(merge: true));

    if (!mounted || isAlreadyActive) return;

    final messenger = ScaffoldMessenger.of(context);
    messenger.hideCurrentSnackBar();
    messenger.showSnackBar(
      SnackBar(content: Text('$locatorName paired')),
    );
  }
